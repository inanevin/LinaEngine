...
namespace tf
{
    ...
    */
    ...
    private:
        ...
        template <typename... ArgsT> Node* _emplace_back(ArgsT&&... args);
    ...
    */
    ...
    // constructor
    inline Runtime::Runtime(Executor& e, Worker& w, Node* p) : _executor{e}, _worker{w}, _parent{p}
    {
    }
    ...
    */
    class Node
    {
        ...
        struct Async
        {
            ...
            template <typename T> Async(T&&, std::shared_ptr<AsyncTopology>);
        ...
        struct SilentAsync
        {
            ...
            template <typename C> SilentAsync(C&&);
        ...
        struct cudaFlow
        {
            ...
            template <typename C, typename G> cudaFlow(C&& c, G&& g);
        ...
        struct syclFlow
        {
            ...
            template <typename C, typename G> syclFlow(C&& c, G&& g);
        ...
        using handle_t = std::variant<std::monostate, // placeholder
    ...
    */
    ...
    // Constructor
    template <typename C> Node::Static::Static(C&& c) : work{std::forward<C>(c)}
    {
    }
    ...
    // Constructor
    template <typename C> Node::Dynamic::Dynamic(C&& c) : work{std::forward<C>(c)}
    {
    }
    ...
    // Constructor
    template <typename C> Node::Condition::Condition(C&& c) : work{std::forward<C>(c)}
    {
    }
    ...
    // Constructor
    template <typename C> Node::MultiCondition::MultiCondition(C&& c) : work{std::forward<C>(c)}
    {
    }
    ...
    template <typename C, typename G> Node::cudaFlow::cudaFlow(C&& c, G&& g) : work{std::forward<C>(c)}, graph{std::forward<G>(g)}
    {
    }
    ...
    template <typename C, typename G> Node::syclFlow::syclFlow(C&& c, G&& g) : work{std::forward<C>(c)}, graph{std::forward<G>(g)}
    {
    }
    ...
    // Constructor
    template <typename T> inline Node::Module::Module(T& obj) : graph{obj.graph()}
    {
    }
    ...
    // Constructor
    template <typename C> Node::Async::Async(C&& c, std::shared_ptr<AsyncTopology> tpg) : work{std::forward<C>(c)}, topology{std::move(tpg)}
    {
    }
    ...
    // Constructor
    template <typename C> Node::SilentAsync::SilentAsync(C&& c) : work{std::forward<C>(c)}
    {
    }
    ...
    // Constructor
    template <typename C> Node::Runtime::Runtime(C&& c) : work{std::forward<C>(c)}
    {
    }
    ...
    // Constructor
    template <typename... Args> Node::Node(Args&&... args) : _handle{std::forward<Args>(args)...}
    {
    }
    ...
    // Move constructor
    inline Graph::Graph(Graph&& other) : _nodes{std::move(other._nodes)}
    {
    }
    ...
    inline void Graph::_clear_detached()
    {
        ...
        auto mid = std::partition(_nodes.begin(), _nodes.end(), [](Node* node) { return !(node->_state.load(std::memory_order_relaxed) & Node::DETACHED); });
