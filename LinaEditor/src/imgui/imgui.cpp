 ...
 The bindings in impl_impl_XXX.cpp files contains many working implementations of a rendering function.
    ...
    void void MyImGuiRenderFunction(ImDrawData* draw_data)
    {
       ...
       for (int n = 0; n < draw_data->CmdListsCount; n++)
       {
          ...
          const ImDrawVert* vtx_buffer = cmd_list->VtxBuffer.m_data;  // vertex buffer generated by Dear ImGui
          const ImDrawIdx* idx_buffer = cmd_list->IdxBuffer.m_data;   // index buffer generated by Dear ImGui
...
{
    ImGuiStorage::ImGuiStoragePair* first = data.m_data;
    ImGuiStorage::ImGuiStoragePair* last = data.m_data + data.Size;
...
void ImGuiStorage::BuildSortByKey()
{
    ...
    if (m_data.Size > 1)
        ImQsort(m_data.m_data, (size_t)m_data.Size, sizeof(ImGuiStoragePair), StaticFunc::PairCompareByID);
}

int ImGuiStorage::GetInt(ImGuiID key, int default_val) const
{
    ImGuiStoragePair* it = LowerBound(const_cast<ImVector<ImGuiStoragePair>&>(m_data), key);
...
{
    ImGuiStoragePair* it = LowerBound(m_data, key);
    if (it == m_data.end() || it->key != key)
        it = m_data.insert(it, ImGuiStoragePair(key, default_val));
    return &it->val_i;
}
...
void ImGuiStorage::SetInt(ImGuiID key, int val)
{
    ...
    if (it == m_data.end() || it->key != key)
    {
        m_data.insert(it, ImGuiStoragePair(key, val));
        return;
    }
    it->val_i = val;
}
...
{
    // Build switch-case from enum with regexp: ImGuiCol_{.*}, --> case ImGuiCol_\1: return "\1";
    switch (idx)
    {
...
{
    // Build the draw list on demand, because they are not frequently used for all viewports
    ImGuiContext& g = *GImGui;
...
void ImGui::NewFrame()
{
    ...
    // Build implicit/fallback window - which we will only render it if the user has added something to it.
...
static void AddWindowToSortBuffer(ImVector<ImGuiWindow*>* out_sorted_windows, ImGuiWindow* window)
{
    ...
    if (window->Active)
    {
        ...
        if (count > 1)
            ImQsort(window->DC.ChildWindows.m_data, (size_t)count, sizeof(ImGuiWindow*), ChildWindowComparer);
...
static void AddDrawListToDrawData(ImVector<ImDrawList*>* out_list, ImDrawList* draw_list)
{
    ...
    IM_ASSERT(draw_list->VtxBuffer.Size == 0 || draw_list->_VtxWritePtr == draw_list->VtxBuffer.m_data + draw_list->VtxBuffer.Size);
    IM_ASSERT(draw_list->IdxBuffer.Size == 0 || draw_list->_IdxWritePtr == draw_list->IdxBuffer.m_data + draw_list->IdxBuffer.Size);
...
static void SetupViewportDrawData(ImGuiViewportP* viewport, ImVector<ImDrawList*>* draw_lists)
{
    ...
    draw_data->CmdLists = (draw_lists->Size > 0) ? draw_lists->m_data : NULL;
    ...
    {
        draw_data->TotalVtxCount += draw_lists->m_data[n]->VtxBuffer.Size;
        draw_data->TotalIdxCount += draw_lists->m_data[n]->IdxBuffer.Size;
    }
}
    ...
    else
    {
        ...
        if (!(flags & ImGuiWindowFlags_NoBackground))
        {
            ...
            if (g.DragDropActive && (g.FrameCount - g.DragDropAcceptFrameCount) <= 1 && g.IO.ConfigDockingTransparentPayload)
                if (g.DragDropPayload.IsDataType(IMGUI_PAYLOAD_TYPE_WINDOW) && *(ImGuiWindow**)g.DragDropPayload.m_data == window)
...
void ImGui::RenderWindowTitleBarContents(ImGuiWindow* window, const ImRect& title_bar_rect, const char* name, bool* p_open)
{
    ...
    ImRect layout_r(title_bar_rect.Min.x + pad_l + LINA_UNDOCKEDWINDOW_TITLE_XOFFSET, title_bar_rect.Min.y + LINA_UNDOCKEDWINDOW_TITLE_YOFFSET, title_bar_rect.Max.x - pad_r, title_bar_rect.Max.y);
...
ImGuiWindow* ImGui::GetTopMostPopupModal()
{
    ...
    for (int n = g.OpenPopupStack.Size - 1; n >= 0; n--)
        if (ImGuiWindow* popup = g.OpenPopupStack.m_data[n].Window)
...
bool ImGui::SetDragDropPayload(const char* type, const void* data, size_t data_size, ImGuiCond cond)
{
    ...
    if (cond == ImGuiCond_Always || payload.DataFrameCount == -1)
    {
        ...
        if (data_size > sizeof(g.DragDropPayloadBufLocal))
        {
            ...
            payload.m_data = g.DragDropPayloadBufHeap.m_data;
            memcpy(payload.m_data, data, data_size);
        }
        else if (data_size > 0)
        {
            // Store locally
            memset(&g.DragDropPayloadBufLocal, 0, sizeof(g.DragDropPayloadBufLocal));
            payload.m_data = g.DragDropPayloadBufLocal;
            memcpy(payload.m_data, data, data_size);
        }
        else
        {
...
void ImGui::LoadIniSettingsFromMemory(const char* ini_data, size_t ini_size)
{
    ...
    char* const buf = g.SettingsIniData.Buf.m_data;
...
static void ImGui::UpdateViewportsNewFrame()
{
    ...
    // Build/update main viewport with current platform position.
    ...
    for (int n = 0; n < g.Viewports.Size; n++)
    {
        ...
        if (n > 0 && viewport->LastFrameActive < g.FrameCount - 2)
        {
            ...
            g.Viewports.erase(g.Viewports.m_data + n);
...
void ImGui::UpdatePlatformWindows()
{
    ...
    // Build/resize/destroy platform windows to match each active viewport.
...
void ImGui::DockContextShutdown(ImGuiContext* ctx)
{
    ...
    for (int n = 0; n < dc->Nodes.m_data.Size; n++)
        if (ImGuiDockNode* node = (ImGuiDockNode*)dc->Nodes.m_data[n].val_p)
            IM_DELETE(node);
}
...
void ImGui::DockContextRebuildNodes(ImGuiContext* ctx)
{
    ...
    DockContextBuildNodesFromSettings(ctx, dc->NodesSettings.m_data, dc->NodesSettings.Size);
...
void ImGui::DockContextUpdateUndocking(ImGuiContext* ctx)
{
    ...
    if (!(g.IO.ConfigFlags & ImGuiConfigFlags_DockingEnable))
    {
        if (dc->Nodes.m_data.Size > 0 || dc->Requests.Size > 0)
...
void ImGui::DockContextUpdateDocking(ImGuiContext* ctx)
{
    ...
    // Build windows for each automatic docking nodes
...
void ImGui::DockContextProcessDock(ImGuiContext* ctx, ImGuiDockRequest* req)
{
    ...
    // Build new node and add existing window to it
    if (node == NULL)
    {
    ...
    {
        // Build tab bar before we call DockNodeMoveWindows (which would attempt to move the old tab-bar, which would lead us to payload tabs wrongly appearing before target tabs!)
...
void ImGui::DockContextProcessUndockNode(ImGuiContext* ctx, ImGuiDockNode* node)
{
    ...
    {
        // In the case of a root node or central node, the node will have to stay in place. Build a new node to receive the payload.
...
static void ImGui::DockNodeRemoveWindow(ImGuiDockNode* node, ImGuiWindow* window, ImGuiID save_dock_id)
{
    ...
    for (int n = 0; n < node->Windows.Size; n++)
        ...
        {
            node->Windows.erase(node->Windows.m_data + n);
            erased = true;
            break;
        }
    IM_ASSERT(erased);
...
static void ImGui::DockNodeUpdate(ImGuiDockNode* node)
{
    ...
    if (central_node_hole)
        if (const ImGuiPayload* payload = ImGui::GetDragDropPayload())
            if (payload->IsDataType(IMGUI_PAYLOAD_TYPE_WINDOW) && DockNodeIsDropAllowed(host_window, *(ImGuiWindow**)payload->m_data))
...
static void ImGui::DockNodeUpdateTabBar(ImGuiDockNode* node, ImGuiWindow* host_window)
{
    ...
    if (tab_bar->Tabs.Size > tabs_unsorted_start)
    {
        ...
        if (tab_bar->Tabs.Size > tabs_unsorted_start + 1)
            ImQsort(tab_bar->Tabs.m_data + tabs_unsorted_start, tab_bar->Tabs.Size - tabs_unsorted_start, sizeof(ImGuiTabItem), TabItemComparerByDockOrder);
    }
...
// Build an explicit dockspace node within an existing window. Also expose dock node flags and creates a CentralNode by default.
...
void ImGui::DockBuilderRemoveNodeChildNodes(ImGuiID root_id)
{
    ...
    if (nodes_to_remove.Size > 1)
        ImQsort(nodes_to_remove.m_data, nodes_to_remove.Size, sizeof(ImGuiDockNode*), DockNodeComparerDepthMostFirst);
    for (int n = 0; n < nodes_to_remove.Size; n++)
...
void ImGui::BeginDockableDragDropTarget(ImGuiWindow* window)
{
    ...
    if (!payload->IsDataType(IMGUI_PAYLOAD_TYPE_WINDOW) || !DockNodeIsDropAllowed(window, *(ImGuiWindow**)payload->m_data))
    {
        EndDragDropTarget();
        return;
    }

    ImGuiWindow* payload_window = *(ImGuiWindow**)payload->m_data;
...
static void ImGui::DockSettingsHandler_WriteAll(ImGuiContext* ctx, ImGuiSettingsHandler* handler, ImGuiTextBuffer* buf)
{
    ...
    dc->NodesSettings.reserve(dc->Nodes.m_data.Size);
    for (int n = 0; n < dc->Nodes.m_data.Size; n++)
        if (ImGuiDockNode* node = (ImGuiDockNode*)dc->Nodes.m_data[n].val_p)
            if (node->IsRootNode())
...
static const char* GetClipboardTextFn_DefaultImpl(void*)
{
    ...
    if (const WCHAR* wbuf_global = (const WCHAR*)::GlobalLock(wbuf_handle))
    {
        ...
        ::WideCharToMultiByte(CP_UTF8, 0, wbuf_global, -1, g.ClipboardHandlerData.m_data, buf_len, NULL, NULL);
    }
    ::GlobalUnlock(wbuf_handle);
    ::CloseClipboard();
    return g.ClipboardHandlerData.m_data;
}
    ...
    for (ItemCount i = 0; i < item_count; i++)
    {
        ...
        for (CFIndex j = 0, nj = CFArrayGetCount(flavor_type_array); j < nj; j++)
        {
            ...
            if (PasteboardCopyItemFlavorData(main_clipboard, item_id, CFSTR("public.utf8-plain-text"), &cf_data) == noErr)
            {
                ...
                CFDataGetBytes(cf_data, CFRangeMake(0, length), (UInt8*)g.ClipboardHandlerData.m_data);
...
void ImGui::ShowMetricsWindow(bool* p_open)
{
    ...
    struct Funcs
    {
        ...
        static void NodeDrawCmdShowMeshAndBoundingBox(ImDrawList* fg_draw_list, const ImDrawList* draw_list, const ImDrawCmd* draw_cmd, int elem_offset, bool show_mesh, bool show_aabb)
        {
            ...
            ImDrawIdx* idx_buffer = (draw_list->IdxBuffer.Size > 0) ? draw_list->IdxBuffer.m_data : NULL;
        ...
        {
            if (!ImGui::TreeNode(label, "%s: %d entries, %d bytes", label, storage->m_data.Size, storage->m_data.size_in_bytes()))
                return;
            for (int n = 0; n < storage->m_data.Size; n++)
            {
                const ImGuiStorage::ImGuiStoragePair& p = storage->m_data[n];
    ...
    if (ImGui::TreeNode("Internal state"))
    {
        ...
        ImGui::Text("HoveredId: 0x%08X/0x%08X (%.2f sec), AllowOverlap: %d", g.HoveredId, g.HoveredIdPreviousFrame, g.HoveredIdTimer, g.HoveredIdAllowOverlap); // m_data is "in-flight" so depending on when the Metrics window is called we may see current frame information or not
