...
#define FOURCC_DXT5 MAKEFOURCCDXT('5')
	...
	void GLRenderDevice::inittest()
	{
		...
		GLuint programID = loadshader("Resources/Shaders/vertex.glsl", "Resources/Shaders/fragment.glsl");
		glUseProgram(programID);

	}

	void GLRenderDevice::looptest()
	{
	...
	int GLRenderDevice::loadshader(const char * vfp, const char * ffp)
	{
		// Create the shaders
		...
		std::ifstream VertexShaderStream(vfp, std::ios::in);
		...
		std::ifstream FragmentShaderStream(ffp, std::ios::in);
